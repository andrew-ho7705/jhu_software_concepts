...
@pytest.mark.db
def test_idempotency(client, mock_llm, connect_to_db, example_duplicate_applicant_data,
                     monkeypatch):
...
def test_simple_query(client, mock_llm, connect_to_db, example_applicant_data, monkeypatch):
    ...
    cur.execute("""
                SELECT program, comments, date_added, url, status, term, llm_generated_program, llm_generated_university 
                FROM test
                """
                )
    entries = cur.fetchall()
