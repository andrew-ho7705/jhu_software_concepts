...
def test_idempotency(client, mock_llm, connect_to_db, example_duplicate_applicant_data, monkeypatch):
    ...
    # Duplicate rows do not create duplicates in database (accidentally pulling the same data should not result in the database acquiring duplicated rows)
    response = client.post("/pull_data")
    assert response.status_code == 200
...
def test_simple_query(client, mock_llm, connect_to_db, example_applicant_data, monkeypatch):
    ...
    # You should be able to query your data to return a dict with our expected keys (the required data fields within M3)
...
@pytest.mark.db
def test_parse_date():
    assert load_data.parse_date("Added on September 17, 2025") == datetime.datetime(int(2025), 9, int(17)).date()
    assert load_data.parse_date("") == None
    assert load_data.parse_date("Added on Sept 17, 2025") == None

@pytest.mark.db
def test_handle_score():
    assert load_data.handle_score("20.25") == 20.25
    assert load_data.handle_score(" ") == None